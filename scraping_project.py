...
next_page = "/"
all_quotes, all_authors, all_bios = [], [], []
...
while next_page:                                          # loop until no next page found
    ...
    # Append this page's results to the master lists
    ...
    time.sleep(1)  # pause 1 second between page requests

# Zip into pairs, shuffle, then unzip so quotes stay matched with correct author/bio
combined = list(zip(all_quotes, all_authors, all_bios))
random.shuffle(combined)
all_quotes, all_authors, all_bios = zip(*combined)

# Loop through each quote/author/bio path as a synced group
for quote, author, bio in zip(all_quotes, all_authors, all_bios):
